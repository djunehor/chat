	...
	*/
	function password($value,$min=6,$max=20) {
		...
		if(!is_int($min) || !is_int($max)) {
		 $output = 'Invalid arguments supplied!';
	 } elseif(strlen($value)<$min) {
		$output = "Password too short! Minimum $min characters."; } elseif(strlen($value)>$max) {
		$output = "Password too long! Maximum $max characters."; } elseif(!preg_match("#[0-9]+#", $value)) {
			$output = "Password must include at least one number!"; } elseif(!preg_match("#[a-z]+#", $value)) {
				$output = "Password must include at least one letter!"; } elseif(!preg_match("#[A-Z]+#", $value)) {
					$output = "Password must include at least one CAPS!"; } elseif( !preg_match("#\W+#", $value)) {
						$output = "Password must include at least one symbol!"; }
return $output;
	}
