...
$type = filter_var($_REQUEST['value'], FILTER_SANITIZE_STRING);
...
case 'del_mile':
@$password= filter_var($_REQUEST['pid'], FILTER_SANITIZE_STRING); 
if(!is_numeric($password)) {$error .= "Invalid MilestoneID. Recieved ID is ".$password; }
...
case 'del_task':
@$password= filter_var($_REQUEST['pid'], FILTER_SANITIZE_STRING); 
if(!is_numeric($password)) {$error .= "Invalid TaskID. Recieved ID is ".$password; }
...
case 'del_con':
@$password= filter_var($_REQUEST['pid'], FILTER_SANITIZE_STRING); 
if(filter_var($password,FILTER_VALIDATE_EMAIL)===false) {$error .= "Invalid Contractor Email"; }
...
case 'del_project':
@$password= filter_var($_REQUEST['pid'], FILTER_SANITIZE_STRING); 
if(!is_numeric($password)) {$error .= "Invalid projectID. Recieved ID is ".$password; }
...
case 'start_project':
@$password= filter_var($_REQUEST['pid'], FILTER_SANITIZE_STRING); 
if(!is_numeric($password)) {$error .= "Invalid projectID."; }
elseif(mysqli_num_rows(mysqli_query($con,"select title from $project_table where projectID='$password'"))!=1){
$error .= 'Project Not Found! <br>';
}
else
...
if(!is_numeric($password)) {$error .= "Invalid TaskID."; }
...
if(strlen($password)<10){ $error .= "Title too short! Minimum 10 characters. <br>"; }
elseif(strlen($password)>100){ $error .= "Title too long! Maximum 100 characters. <br>"; }
elseif(!preg_match("#[a-z]+#", $password)){ $error .= "Title must include at least one letter! <br>"; }
...
if(strlen($password)<10){ $error .= "Title too short! Minimum 10 characters. <br>"; }
elseif(strlen($password)>100){ $error .= "Title too long! Maximum 100 characters. <br>"; }
elseif(!preg_match("#[A-Za-z]+#", $password)){ $error .= "Title must include at least one letter! <br>"; }
