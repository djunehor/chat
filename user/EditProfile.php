...
<form id="update_profile" role="form" enctype="multipart/form-data">
         ...
         <!--     <p>Try the keyboard navigation by clicking arrow left or right!</p> -->
                ...
                <input class="form-control" name="fullname" placeholder="Enter full name" value="<?php echo $user['full_name']; ?>" type="text">
            </section><br>
            <section>
          <!--    <p>Wonderful transition effects.</p>  -->
         
                <label class="form-control-label">Phone: <span class="tx-danger">*</span></label>
                <input onchange="check_number(this.value)" id="number" class="form-control" name="phone" value="<?php echo $user['phone']; ?>" type="number" />
	...
	$("#update_profile").on('submit',(function(e) {
		...
		$.ajax({
			url: "<?php echo $website_url; ?>/ajax_form_process?value=mprofile",
			type: "POST",
...
function check_photo(value){
	$.post("<?php echo $website_url; ?>/ajax_check_file?value=photo",{uphoto:value},function(data){
		if(data.length != 0){
